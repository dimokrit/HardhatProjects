...
contract Girand_Hero_Medusa is Ownable, ReentrancyGuard, ERC2981, ERC721A {
    ...
    constructor(string memory baseURI, string memory name, string memory symbol, uint96 royalty, bytes32 _merkleRoot) ERC721A(name, symbol) {
        baseTokenURI = baseURI;
        activeDrop = 1;
        drops[activeDrop].preMintMaxSupply = 5;
        drops[activeDrop].maxDropSupply = 5;
        drops[activeDrop].timeMintAfterPreMint = 100;
    ...
    */
    ...
    ///@notice Get data of drop
    ///@param dropId is drop id for info
     function getDropData(uint256 dropId) external view returns (
        uint256 totalSupply, 
        uint256 preMintMaxSupply, 
        uint256 maxDropSupply, 
        uint256 timeMintAfterPreMint,
        ...
        bytes32 merkleRoot) {
       return (
        drops[dropId].totalSupply,
        drops[dropId].preMintMaxSupply,
        drops[dropId].maxDropSupply,
        drops[dropId].timeMintAfterPreMint,
        drops[dropId].preMintStarted,
        drops[dropId].whiteList,
        drops[dropId].merkleRoot
       );
    }

    ///@notice Get info from NFTtracker
    ///@param dropId is drop id for info
    ///@param trackAddress is address wich you want to track
     function getDropNFTtrackerInfo(uint256 dropId, address trackAddress) external view returns (bool nftMinted) {
       return (drops[dropId].NFTtracker[trackAddress]);
    }
